...
setGeneric("adjust.methylC", function(mc,hmc,save.db=FALSE,...,chunk.size=1e6) standardGeneric("adjust.methylC") )

#' @rdname adjust.methylC
#' @aliases adjust.methylC,methylRaw,methylRaw-method
setMethod("adjust.methylC", c("methylRaw","methylRaw"),function(mc,hmc,save.db=FALSE,...){
...
#                         } else { dbtype <- args$dbtype }
    ...
    # create methylRawDB
    obj <- makeMethylRawDB(df=data[,1:7],dbpath=dbdir,dbtype="tabix",sample.id=paste0(mc@sample.id,suffix),
...
setMethod("adjust.methylC", c("methylRawList","methylRawList"),function(mc,hmc,save.db=FALSE,...){
  ...
  new("methylRawList",my.list,treatment=mc@treatment )
    ...
    my.list=list()
    for(i in 1:length(mc)){
      my.list[[i]]=adjust.methylC(mc[[i]],hmc[[i]],save.db=TRUE,dbdir=basename(dbdir),...)
    }
