...
#covariates=data.frame(age=c(10,20,30,40,20,20),
#                      batch=c("a","a","a","b","b","b"))
#vars=cbind(treatment,covariates)
#fmla<-as.formula(paste("~ ", paste(colnames(vars), collapse= "+")))

#' @param overdispersion MN McCullagh and Nelder (1989) recommendation for 
#'        overdispersion correction. 
#' @param effect
logRegDF<-function(df,treatment,covariates=NULL,overdispersion=c("none","MN","shrinkMN"),
                   effect=c("wmean","mean","predicted"),parShrinkNM=list(),mc.cores=1)
{
  ...
  res=simplify2array(
          mclappy(cntlst,logReg,formula,vars,overdispersion,effect,parShrinkNM,mc.cores))
  
  # return the data frame part of methylDiff
}

# treatment=as.factor(c(0,0,0,1,1,1))
# treatment=as.factor(c("a","a","b","b","c","d"))
# treatment=as.factor(c(1,1,2,2,3,3))
# covariates=data.frame( batch=c("a","b","a","a","a","b"))
# covariates=NULL
# vars=ifelse(is.null(covariates),data.frame(treatment),data.frame(treatment,covariates) )
# formula<-as.formula(paste("~ ", paste(colnames(vars), collapse= "+")))
# counts=c(30,30,45,10,10,8,80,70,60,90,95,99)
...
logReg<-function(counts,formula,vars, overdispersion=c("none","MN","shrinkMN"),
  ...
  #fmla<- paste("~ ", paste(colnames(vars), collapse= "+")) 
  modelMat<-model.matrix( formula ,as.data.frame(vars) )
  obj=glm.fit(modelMat,prop,weights=w,family=binomial(link=logit))
  
  
  # check the confidence interval for predictions
  ## mod=glm(cbind(y,w-y)~treatment,family= quasibinomial)
  ## pred=predict(mod,newdata=data.frame(treatment=as.factor(c(0,1))),se.fit=T,type="response")
  ## plot(c(0,0,0,1,1,1),y/w,ylim=c(0,1),pch=19)
  ## points(c(0,1),pred$fit+2*pred$se.fit,pch=6,col="red")
  ## points(c(0,1),pred$fit-2*pred$se.fit,pch=2,col="red")
  ## bmod=betabin(cbind(y,w-y)~treatment,~ 1,data = data.frame(y,w,treatment))
  ## bpred=predict(bmod,newdata=data.frame(treatment=as.factor(c(0,1))),se.fit=T)
  ## points(c(0,1),bpred$fit+2*bpred$se.fit,pch=6,col="pink")
  ## points(c(0,1),bpred$fit-2*bpred$se.fit,pch=2,col="pink")
  
  mu=fitted(obj)
  #resids <- (y-w*mu)/sqrt(mu*(w-w*mu)) # pearson residuals
  nprm=length(obj$coef) # number of parameters fitted
  #phi <- sum( resids^2 )/(length(w)-prm) # correction factor
  ...
  phi=switch(overdispersion,
                    ...
                    MN={
                      mu=fitted(obj)
                      resids <- (y-w*mu)/sqrt(mu*(w-w*mu)) # pearson residuals
                      phi=sum( resids^2 )/(length(w)-nprm) # correction factor
                      ifelse(phi>1,phi,1)
                    },
                    shrinkMN=1)
  ...
  ## needed for T-values of coeff
  ## not really needed for LRT
  ...
  #ddf=1 # difference in df
  #p.value <- 1-pchisq(deviance/phi,df=ddf)
  ...
  #p.value = pf(deviance/phi, ddf,df.scale=(length(w)-prm), lower.tail = FALSE))
  test=match.arg(test)
