...
\texttt{removeComp}. In addition to these, if you have found other ways to correct for batch effects and obtained a corrected
...
to the \texttt{reconstruct} function. All of these functions work on a \texttt{methylBase}
