...
Most of the time bisulfite sequencing experiments have test and control samples. A test sample can be from a disease tissues while the control sample can be from healthy tissues. You can read a set of methylation call files that have test/control conditions giving \texttt{treatment} vector option as follows: 
