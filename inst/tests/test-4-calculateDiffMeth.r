library("methylKit")
context("calculateDiffMeth and annotateWithGenicPart checks")
...
test_that("annotate.WithGenicParts", {
    expect_that(annotateWithGenicParts(myDiff,gene.obj),
        is_a('annotationByGenicParts'))
})


test_that("annotate.WithGenicParts", {
    expect_that(annotateWithFeatureFlank(myDiff,cpg.obj$CpGi,cpg.obj$shores),
