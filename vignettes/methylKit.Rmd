...
Overdispersion occurs when there is more variablity in the data than assumed
by the distribution. In the logistic regression model, the response variable
methylation values for sample groups are expected to have binomial distribution:
$meth_i ~ Bin(n_i, \pi_i)$.
Therefore, the methylation values will have the variance $\mu_i (n_i-\mu_i)/n_i$ 
and mean $\mu_i$. $n_i$ is the coverage for the CpG or a region and $\pi_i$ is 
the underlying methylation proportion. Overdispersion occurs when the variance
of $meth_i$ is greater than $\mu_i (n_i-\mu_i)/n_i$. This can be corrected by
calculating a scaling parameter $\phi$ and adjusting the variance 
as in $\phi \mu_i (n_i-\mu_i)/n_i$. `calculateDiffMeth` can calculate that
...
By default,the overdispersion correction is not applied. 
...
cases, the procedure tests if the full model (the model where treatmet is 
